...
const ClientManagement = () => {
    ...
    useEffect(() => {
        if (!selectedClient || !isAccountingModalOpen || plans.length === 0) return;
        
        // Match by Name or Tier (Case Insensitive)
        ...
        setPaymentForm(prev => ({
            ...
            // Only auto-update amountPaid if it hasn't been manually edited or matches previous total
            amountPaid: (prev.amountPaid === "" || parseFloat(prev.amountPaid) === prev.totalPayable) ? payable : prev.amountPaid
        }));
    }, [paymentForm.planTier, paymentForm.planDuration, paymentForm.discount, isAccountingModalOpen, selectedClient, plans]);
    ...
    const handleRecordPayment = async (e) => {
        ...
        const payload = {
            ...paymentForm,
            // If not extending, the 'payable' for the plan is 0, so entire amountPaid settles balance
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Update the active state so debt/balance UI refreshes immediately
                if (data.client) {
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            setPaymentForm(prev => ({
                                                ...prev,
                                                planTier: client.plan || 'SILVER',
                                                planDuration: client.planDuration || '1m',
                                                discount: 0,
                                                amountPaid: '',
                                                extendSubscription: false // Reset to false by default
                                            }));
                    ...
                    <div className="modal-content animate-slideUp premium-glass" style={{ maxWidth: '950px', border: '1px solid var(--border)', padding: '2.5rem', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '2.5rem' }}>
                        ...
                        <div>
                            ...
                            <form onSubmit={handleRecordPayment} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                ...
                                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '1.25rem', borderRadius: '16px', border: '1px solid var(--border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                                    <div className="input-group">
                                        <label style={{ color: 'var(--text-muted)', fontSize: '0.7rem' }}>Base Price</label>
                                        <div style={{ fontSize: '1.25rem', fontWeight: 800 }}>Rs. {(paymentForm.baseAmount || 0).toLocaleString()}</div>
                                    </div>
                                    <div className="input-group">
                                    ...
                                    <div className="input-group" style={{ gridColumn: 'span 2', borderTop: '1px solid var(--border)', paddingTop: '0.75rem', marginTop: '0.25rem' }}>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div className="input-wrapper">
                                            ...
                                            >
                                                ...
                                                <option value="QR Code" style={{ background: '#1a1a1a' }}>QR Code</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                                    <button type="button" className="btn-ghost" onClick={() => setIsAccountingModalOpen(false)} style={{ flex: 1, padding: '0.8rem' }}>Cancel</button>
